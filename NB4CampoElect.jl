...
# ╔═╡ 5d070bf0-f85b-11ef-225a-bd56e9935fff
begin
	using PlutoUI
...
# ╔═╡ e63e15d6-2ee6-4592-9d29-155e5cc6ffd1
md"""
En muchos casos suele ser más práctico pensar que una carga $Q_1$ altera todo el espacio a su alrededor al crear un campo vectorial $\vec{E}_1(x,y,z)$ y este campo es el que interactua con otra carga $Q_2$ de tal forma que la fuerza sobre la carga $Q_2$ ejercida por la carga $Q_1$ es:

$\vec{F}_{12} = Q_2 \vec{E}_1(x_2,y_2,z_2)$

donde el vector posición de la carga $Q_2$ es $\vec{r}_2=(x_2,y_2,z_2)$.
"""

# ╔═╡ 6aad10aa-15d9-4064-b380-6e696b4fa6a4
md"""
El campo $\vec{E}_1(x,y,z)$ se conoce como el campo eléctrico creado por la carga $Q_1$ mientras que $\vec{E}_1(x_2,y_2,z_2)$ es dicho campo evaluado en la posición $\vec{r}_2=(x_2,y_2,z_2)$.
"""

# ╔═╡ 4c6e472b-3231-4100-9a32-0f15a7d1d5d9
md"""
Para encontrar $\vec{E}_1(x_2,y_2,z_2)$ simplemente se despeja de la ecuación anterior

$\vec{E}_1(x_2,y_2,z_2) = \frac{\vec{F}_{12}}{Q_2}$

Ahora recordemos que la fuerza que ejerce la carga $Q_1$ sobre $Q_2$ está dada por la ley de Coulomb

$\vec{F}_{12} = K \frac{Q_1Q_2}{||\vec{r}_2-\vec{r}_1||^3} (\vec{r}_2-\vec{r}_1)$

por lo tanto

$\begin{align}
\vec{E}_1(x_2,y_2,z_2) &=  K \frac{Q_1\cancel{Q_2}}{\cancel{Q_2}||\vec{r}_2-\vec{r}_1||^3} (\vec{r}_2-\vec{r}_1) \\
\vec{E}_1(x_2,y_2,z_2) &=  K \frac{Q_1}{||\vec{r}_2-\vec{r}_1||^3} (\vec{r}_2-\vec{r}_1)
\end{align}$

Si consideramos que la carga $Q_2$ se puede ubicar en cualquier punto del espacio exceptuando la posición de la carga $Q_1$ entonces se puede decir que el campo eléctrico generado por la carga $Q_1$ es

$\begin{align}
\vec{E}_1(x,y,z) &=  K \frac{Q_1}{||\vec{r}-\vec{r}_1||^3} (\vec{r}-\vec{r}_1)
\end{align}$

donde $\vec{r}_1$ es el vector posición de carga $Q_1$ y $\vec{r} = (x,y,z)$ es el vector posición de un punto arbitrario en el espacio.
"""
...
md"""!!! info "📑 Definición: Campo Eléctrico"
	El campo eléctrico generado por una carga $Q_i$ está dado por
...
# ╔═╡ df8f536b-64ff-4fec-b579-8f3569a9c0ad
md"""!!! info "📚 Nota 1:" 

	Note que en lugar de pensar que una carga ejerce una fuerza sobre otra, estamos pensando que una carga crea un campo vectorial que interactua con la otra carga. Dicho de otra forma, consideramos el campo eléctrico como un intermediario entre las cargas.

	Si bien esta forma de introducir el campo eléctrico es meramente matemática y aparentemente por pura conveniencia, es importante aclarar que el campo eléctrico es algo que existe físicamente y en algunos casos puede existir y propagarse sin necesidad de una carga que lo genere.
...
md"""
Si se deseara graficar el campo eléctrico de una carga puntual $Q_1$ en cada punto del espacio alrededor de dicha carga se obtendria lo siguiente:
"""

# ╔═╡ ae424b6d-c29f-4960-b86a-0deb7726a0e0
md"""
Q1 = $(@bind Q1 PlutoUI.Slider(-2:0.1:2, default = 1, show_value = true))
"""

# ╔═╡ 5fa70483-65a0-4bb5-86f3-7fe936038f94
begin

# Define the charge Q1
#Q1 = 1.0  # Charge magnitude (arbitrary units)
k = 1.0   # Coulomb constant (set to 1 for simplicity)

# Define grid for electric field visualization
x_range = y_range = LinRange(-2, 2, 15)
X, Y = [p[1] for p in Iterators.product(x_range, y_range)], 
       [p[2] for p in Iterators.product(x_range, y_range)]

# Compute electric field vectors
origins = Point2f[]  # Store points for arrow origins
vectors = Vec2f[]    # Store arrow directions

charge_pos = (0.0, 0.0)  # Charge Q1 is at origin

for i in eachindex(X, Y)
    dx, dy = X[i] - charge_pos[1], Y[i] - charge_pos[2]
    r = sqrt(dx^2 + dy^2)
    
    if r > 0.2  # Avoid singularity at charge position
        E = k * Q1 / r^2
        push!(origins, Point2f(X[i], Y[i]))      # Arrow starting point
        push!(vectors, Vec2f(E * dx / r, E * dy / r))  # Arrow direction
    end
end
...
fig1 = Figure()
ax1 = Axis(fig1[1, 1], title = L"Campo eléctrico debido a una carga $Q_1$", xlabel="x", ylabel="y")

# Plot the charge as a red point
scatter!(ax1, [0], [0], color=:red ,markersize=15, label = L"Q_1" )

# Plot the electric field vectors

arrows2d!(ax1, origins, vectors, color=:black, colormap = :blues, lengthscale=0.1, label = L"$\vec{E}_1(\vec{r})$")
...
md"""
Note que el campo eléctrico es más intenso cuanto más cerca este de la carga, esto se aprecia ya que la longitud de los vectores es mayor a medida que se acerca a la carga. También es posible ver como cambian de direción cuando cambia el signo de la carga.
"""

# ╔═╡ bdeaf75c-6acf-42c0-9613-d8c8e018a1d0
md"""
Un gran inconveniente de estos gráficos es que los vectores cerca de la carga acaban siendo tan grandes que se superponen con otros y dificulta la visialuzación. Además son bastante engorrosos de dibujar. Por consiguiente en la práctica se usan las líneas de campo mostradas a continuación:
"""
...
for θ in θ_range_strplt
	lines!(ax2, r_range_strplt.*cos(θ), r_range_strplt.*sin(θ), color = :black, linewidth = 2.5 )
	arrows2d!(ax2, [cos(θ)], [sin(θ)], [cos(θ)], [sin(θ)], color = :black,  label = L"$\vec{E}_1(\vec{r})$" )
	arrows2d!(ax2, [cos(θ)/2], [sin(θ)/2], [cos(θ)/2], [sin(θ)/2], color = :black,  label = L"$\vec{E}_1(\vec{r})$")
end
...
begin
	...
	# Plot the electric field as streamlines
	for θ in θ_range_strplt
		lines!(ax31, r_range_strplt.*cos(θ), r_range_strplt.*sin(θ), color = :black, linewidth = 2.5 )
		arrows2d!(ax31, [cos(θ)], [sin(θ)], [cos(θ)], [sin(θ)], color = :black,  label = L"$\vec{E}_1(\vec{r})$" )
		arrows2d!(ax31, [cos(θ)/2], [sin(θ)/2], [cos(θ)/2], [sin(θ)/2], color = :black,  label = L"$\vec{E}_1(\vec{r})$" )
	end
	...
	# Plot the electric field as streamlines
	for θ in θ_range_strplt
		lines!(ax32, r_range_strplt.*cos(θ), r_range_strplt.*sin(θ), color = :black, linewidth = 2.5 )
		arrows2d!(ax32, [3*cos(θ)], [3*sin(θ)], [-cos(θ)], [-sin(θ)], color = :black,  label = L"$\vec{E}_1(\vec{r})$" )
		arrows2d!(ax32, [2*cos(θ)/2], [2*sin(θ)/2], [-cos(θ)/2], [-sin(θ)/2], color = :black,  label = L"$\vec{E}_1(\vec{r})$" )
	end
...
manifest_format = "2.0"
project_hash = "6690fb3acd44be9993eb8560536c6d9de549e4af"
