...
# ╔═╡ 811744bd-9a73-4b7d-92f6-f74120af3c86
md"# Sistemas de Coordenadas"

# ╔═╡ dac0fce2-9dea-4b2c-94b7-a7fe6182ad61
md"## Coordenadas Cartesianas"

# ╔═╡ 0780c6f4-d16c-4b1b-b64a-91c6c6769bc7
md" Recordemos que en coordenadas cartesianas todo punto en el espacio lo podemos describir con una tripleta de números, por ejemplo (3,2,1). Cada número nos indica cuanto se debe desplazar a lo largo de los ejes x, y y z. Por consiguiente, un punto arbitrario en el espacio lo representamos como (x,y,z)."

# ╔═╡ e11e8386-c0b3-43b0-b846-c3d6a0ae1175
md"A continuación podrás ver como se ubica un punto en el espacio en coordenadas cartesianas. Observa como cambia la posición del punto a medida que alteras los valores de las coordenadas x,y,z:"

# ╔═╡ 71ec5edf-1552-411c-9e6d-d52ca6ae8bcb

# Create sliders for x, y, and z
@bind cartCoord PlutoUI.combine() do Child
	md""" x= $(Child( Slider(-10:10; default=2, show_value=true))), y= $(Child( Slider(-10:10; default=2, show_value=true))), z= $(Child( Slider(-10:10; default=2, show_value=true)))"""

end

# ╔═╡ 784617f2-762d-49c7-a35e-b50d9b8ac4db
begin
	#plotly()
	# Create the 3D plot
plot(cartCoord, seriestype = :scatter3d, label = "Punto: $cartCoord", legend = true, markersize = 5,
...
md"""Por otro lado, en este sistema las coordenadas pueden tomar los siguientes valores:
...
# ╔═╡ c60563b8-014b-4e16-8975-02f22f7b9ab9
md"Y un vector  $\vec{A}$  en el espacio se pueden expresar como:

$(A_x,A_y,A_z) \quad  o  \quad A_x \hat{a}_x + A_y \hat{a}_y + A_z \hat{a}_z$

donde:

$\hat{a}_x = \begin{pmatrix} 1 \\ 0 \\ 0 \end{pmatrix}, \qquad \hat{a}_y = \begin{pmatrix} 0 \\ 1 \\ 0 \end{pmatrix} \quad y \quad \hat{a}_z = \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix}$
...
md"## Coordenadas Cilíndricas "

# ╔═╡ b1132e89-589b-4f6a-b2de-afc0ae0e0fc4
md"Las coordenadas cilíndricas tambien describen todo punto en el espacio con una tripleta de números $(\rho, \phi, z)$. En este caso $\rho$ representa la distancia del punto hacia el eje z, $\phi$ es una coordenada angular que permite rotar el punto alrededor del eje z y z es la misma coordenada usada en las coordenadas cartesianas."

# ╔═╡ c946ae41-df8c-4ca7-b7f5-096ca5c0b2b6
md"A continuación podrás ver como se ubica un punto en el espacio en coordenadas cilíndricas. Observa como cambia la posición del punto a medida que alteras los valores de las coordenadas $(\rho, \phi, z)$:"
...
# ╔═╡ 93f21700-9cbd-4997-af3a-7ff402be8db3
begin

	#Transform cilindrical coordinates to cartesian coordinates
x_plotCilind = cilindCoord[1] * cos(cilindCoord[2])
y_plotCilind = cilindCoord[1] * sin(cilindCoord[2])
z_plotCilind = cilindCoord[3]
	
	# Create the 3D plot
plot((x_plotCilind,y_plotCilind,z_plotCilind), seriestype = :scatter3d, label = "Punto: $cilindCoord", legend = true, markersize = 5,
	     xlims = (-10, 10), ylims = (-10, 10), zlims = (-10, 10), 
	     xlabel = "X", ylabel = "Y", zlabel = "Z", 
	     background_color = :white, 
	     grid = true, 
...
plot!([x_plotCilind, 0], [y_plotCilind, 0], [z_plotCilind, z_plotCilind], color=:black, linewidth=2, label = nothing)
...
t_plotCilind = LinRange(0, cilindCoord[2], 100)  # Parametric range for the angle φ
curve_x_plotCilind = cilindCoord[1] * cos.(t_plotCilind)
curve_y_plotCilind = cilindCoord[1] * sin.(t_plotCilind)
curve_z_plotCilind = zeros(100)
...
# Add the unit vectors at the point (x, y, z) in cylindrical coordinates

# Radial unit vector (e_ρ)
e_rho_x = cilindCoord[1] * cos(cilindCoord[2])  # Along the X axis in the XY plane
e_rho_y = cilindCoord[1] * sin(cilindCoord[2])  # Along the Y axis in the XY plane
plot!([x_plotCilind, x_plotCilind + vector_scale * cos(cilindCoord[2])], 
      [y_plotCilind, y_plotCilind + vector_scale * sin(cilindCoord[2])], 
      [z_plotCilind, z_plotCilind], color=:purple, linewidth=2, label = "\$\\hat{a}_\\rho\$")

# Add label for radial unit vector
annotate!(x_plotCilind + vector_scale * cos(cilindCoord[2]), 
          y_plotCilind + vector_scale * sin(cilindCoord[2]), 
          z_plotCilind + 2, text("\$\\hat{a}_\\rho\$", :purple, 12))

# Azimuthal unit vector (e_φ)
e_phi_x = -sin(cilindCoord[2])  # Along the negative Y direction in the XY plane
e_phi_y = cos(cilindCoord[2])   # Along the positive X direction in the XY plane
plot!([x_plotCilind, x_plotCilind + vector_scale * e_phi_x], 
      [y_plotCilind, y_plotCilind + vector_scale * e_phi_y], 
      [z_plotCilind, z_plotCilind], color=:orange, linewidth=2, label = "\$\\hat{a}_\\phi\$")

# Add label for azimuthal unit vector
annotate!(x_plotCilind + vector_scale * e_phi_x, 
...
md" Además, note que los vectores unitarios $\hat{a}_\rho$, $\hat{a}_\phi$ y $\hat{a}_z$ son mutuamente ortogonales y 
...
# ╔═╡ ee728be8-b702-407b-9849-3b59c758eb5e
md"El lector debe familiarizarse con las distintas formas de escribir vectores. Cuando se dice que los vectores anteriores están expresados en coordenadas cartesianas se refieren a lo siguiente:

$\hat{a}_\rho =\cos(\phi) \hat{a}_x+\sin(\phi)\hat{a}_y+0\hat{a}_z$
$\hat{a}_\phi =-\sin(\phi) \hat{a}_x+\cos(\phi)\hat{a}_y+0\hat{a}_z$
$\hat{a}_z=0 \hat{a}_x+0\hat{a}_y+1\hat{a}_z$"

# ╔═╡ 2b3d5e2f-4048-4e9e-98b0-49e5c669d720
...
md"""
### Transformaciones entre los sistemas coordenadas cilíndricos y cartesianos.
"""
...
md"""
Aunque dichas transformaciones se pueden obtener usando geometría, se prefiere dejar ese trabajo al lector y comprobar que son las siguientes:
"""
...
md"""
Para entender bien como transformar vectores primero veamos como expresar un vector en coordenadas cilíndricas. Un vector $\vec{A}$ se puede expresar en coordenadas cilindricas como:

$(A_\rho,A_\phi,A_z) \quad  o  \quad A_\rho \hat{a}_\rho + A_\phi \hat{a}_\phi + A_z \hat{a}_z$

¿Facíl no? se expresan igual forma que en coordenadas cartesianas, lo que cambia son los vectores unitarios. 

Ahora si se preguntan como calcular su magnitud, de la definición de magnitud de un vector a partir del producto punto $\left(|\vec{A}|=\sqrt{\vec{A}\cdot\vec{A}} \right)$, y de las propiedades de los vectores unitarios de este sistema de coordenadas se tiene que:
...
manifest_format = "2.0"
project_hash = "6430fb939207fd6ff0e8c2b013de86f465855306"
